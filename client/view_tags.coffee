Template.view_tags.helpers
    theme_tag_class: -> if @valueOf() in selected_theme_tags.array() then 'active' else ''
    location_tag_class: -> if @valueOf() in selected_location_tags.array() then 'active' else ''
    intention_tag_class: -> if @valueOf() in selected_intention_tags.array() then 'active' else ''
